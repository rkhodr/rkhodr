import java.io.*;   // allows us to read from a file
 ...
 public class Sudoku {              
      ...
      * has all empty cells.
      */
     public Sudoku() { // CONSTRUCTOR: No Arguements
        ...
        rowHasVal = new boolean[9][10]; // Index 0 is unused to represent 1-9
        colHasVal = new boolean[9][10];
     }
     ...
     // Returns true if the move in allowed, false otherwise
     private boolean isValidMove(int num, int row, int col) {
        if (subgridHasVal[row/3][col/3][num]){
            return false;
        }
        if (rowHasVal[row][num]){
            return false;
        }
        if (colHasVal[col][num]){
            return false;
        }
     ...
     private boolean solveRB(int n) {
        ...
        for (int num = 1; num <= 9; num++) {
            if (isValidMove(num, row, col)) {
                placeVal(num, row, col);
                if (solveRB(n + 1)) {
                    return true; 
                }
                 removeVal(grid[row][col],row, col); 
            }
        }
